...
class unpack(Command):
    ...
    def execute(self):
        ...
        exts = ['7z','ace','ar','arc','bz2?','cab','cpio','cpt','deb','dgc','dmg','gz','iso','jar','msi','pkg','rar','shar','tar','tgz','xar','xpi','xz','zip']
        if self.fm.thistab.get_selection():
            sel = self.fm.thistab.get_selection()
            for f in sel:
                if f.is_file:
                    if f.extension in exts:
                        self.fm.execute_console(f"shell atool --extract --each -- {f.path}")
                    else:
                        self.fm.notify('no archive or not supported', bad=True)
                else:
                    self.fm.notify('not a file to extract', bad=True)
...
class fd_search(Command):
    ...
    def execute(self):
        ...
        if not 'fdfind' in get_executables():
        ...
        target = target.split(' ')
        self.fm.notify(str(target), bad=True)
        process = subprocess.Popen(['fdfind', result_sep['arg'], depth, *target],
